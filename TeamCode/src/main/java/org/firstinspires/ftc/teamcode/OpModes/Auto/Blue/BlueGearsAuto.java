...
@Autonomous (name = "Blue Gears", group = "Blue")
public class BlueGearsAuto extends LinearOpMode {
    ...
    public void runOpMode() {
        ...
        //TODO: CHANGE THIS!!!!
        drive.setPoseEstimate(poses.blueGearsStartingPoint);

        Trajectory ringDetectionMovement = drive.trajectoryBuilder(poses.blueGearsStartingPoint, Math.toRadians(0))
                .splineToConstantHeading(new Vector2d(-22, 54), Math.toRadians(0))
                .build();

        Trajectory boxAMovements = drive.trajectoryBuilder(new Pose2d(-22, 54, Math.toRadians(-90)), Math.toRadians(0))
                .splineToSplineHeading(poses.blueBoxABottomDrop, Math.toRadians(90))
                .build();

        Trajectory powerShootAMovement = drive.trajectoryBuilder(poses.blueBoxABottomDrop, Math.toRadians(180))
                .splineToLinearHeading(new Pose2d(10, 6, 0), Math.toRadians(0))
                .build();

        Trajectory boxBMovements = drive.trajectoryBuilder(new Pose2d(-22, 54, Math.toRadians(-90)), Math.toRadians(0))
                .splineToConstantHeading(new Vector2d(45, 58), Math.toRadians(0))
                .build();

        Trajectory powerShotBMovement = drive.trajectoryBuilder(new Pose2d(40, 58, Math.toRadians(-90)), Math.toRadians(180))
                .splineTo(new Vector2d(5, 30), Math.toRadians(-90))
                .splineToLinearHeading(new Pose2d(5, 12, Math.toRadians(0)), Math.toRadians(180))
                .build();

        Trajectory boxCMovements = drive.trajectoryBuilder(new Pose2d(-22, 54, Math.toRadians(-90)), Math.toRadians(0))
                .splineToSplineHeading(new Pose2d(40, 60, Math.toRadians(10)), Math.toRadians(0))
                .build();

        Trajectory powerShotCMovement = drive.trajectoryBuilder(new Pose2d(40, 56, Math.toRadians(0)), Math.toRadians(180))
                .splineToConstantHeading(new Vector2d(5, 8), Math.toRadians(-90))
                .build();
        ...
        Trajectory park = drive.trajectoryBuilder(new Pose2d(10, 6, Math.toRadians(7.5)), Math.toRadians(0))
                .forward(5)
                .build();
